...
print(summary(sex_mod))

# 2) Prediction grid across observed age range
...
# 3) Predictions with 95% CI (response scale = probability)
sex_pred_ci <- ggpredict(sex_mod, terms = c("mom_age [all]", "mom_age_group")) %>%
  ...
  # Restrict predictions to valid ranges for each group
  dplyr::filter(
    (mom_age_group == "young" & mom_age < 4) |
    (mom_age_group == "prime" & mom_age >= 4 & mom_age <= 8) |
    (mom_age_group == "old" & mom_age > 8)
...
# 4) Observed summaries to overlay
sex_summ <- otterdat2 %>%
  group_by(mom_age, mom_age_group) %>%
  summarise(
  ...
  # small horizontal dodge by group for readability (matches other plot)
  mutate(mom_age = mom_age + 0.2 * ifelse(mom_age_group == "prime", 0,
                                   ifelse(mom_age_group == "young", -1, 1)))

# 5) Plot (same styling, same thresholds at 4 and 8)
sex_fig <- ggplot() +
  geom_pointrange(
    data = sex_summ,
    aes(x = mom_age, y = mean_male,
  ...
  labs(
    title = "Probability of Producing Male Pups vs. Mother's Age",
    x = "Mother's Age (years)",
...
```{r}
# 1) Model: Negative Binomial GLMM (handles overdispersion)
wean_days_mod <- glmer.nb(
  wean_days ~ mom_age + mom_age_group + (1 | Otterno) + (1 | year),
  data = otterdat2,
  control = glmerControl(optimizer = "bobyqa")
)
print(summary(wean_days_mod))

# 1a) Quick overdispersion check (should be ~1; >>1 indicates overdispersion)
overdisp_fun <- function(model) {
  rdf <- df.residual(model)
  rp  <- residuals(model, type = "pearson")
  Pearson.chisq <- sum(rp^2)
  c(chisq = Pearson.chisq, ratio = Pearson.chisq / rdf, rdf = rdf)
}
print(overdisp_fun(wean_days_mod))

# 2) Prediction grid across observed age range
wean_days_grid <- tidyr::expand_grid(
  mom_age = seq(min(otterdat2$mom_age, na.rm = TRUE),
                max(otterdat2$mom_age, na.rm = TRUE), by = 1)
) %>%
  mutate(
    mom_age_group = case_when(
      mom_age < 4 ~ "young",
...
# 3) Predictions with 95% CI (on response scale = days)
...
# 4) Observed summaries for overlay points/error bars
...
# 5) Plot (predicted lines should align much better than Poisson)
wean_days_fig <- ggplot() +
  ...
  labs(
    ...
    subtitle = "Predicted weaning days from glmer.nb (Negative Binomial)",
    x = "Mother's Age (years)",
